...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="orderRegister/orderRegister.do" method="post" name="Form" id="Form">
					...
					<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
						<thead>
							<tr>
								...
								<th class="center">维修状态</th>
								<th class="center">维修内容</th>
								<th class="center">维修过程描述</th>
						...
						<!-- 开始循环 -->	
						<c:choose>
							<c:when test="${not empty orderRegisterList}">
								<c:if test="${QX.cha == 1 }">
								<c:forEach items="${orderRegisterList}" var="var" varStatus="vs">
									<tr>
										...
										<td class="center">${var.repairContent}</td>
										<td class="center">${var.repairProcess}</td>
										<c:if test="${var.isToRepair == 0 }">
											<td class="center">未送修</td>
										</c:if>
										<c:if test="${var.isToRepair == 1 }">
											<td class="center">已送修</td>
										</c:if>
										<c:if test="${var.isReplace == 0 }">
											<td class="center">未更换</td>
										</c:if>
										<c:if test="${var.isReplace == 1 }">
											<td class="center">已更换</td>
										</c:if>
										<c:if test="${var.isStartBack == 0 }">
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			 addOrEdit('<%=basePath%>orderRegister/goAdd.do','维修登记');
		}
		...
		function del(Id,Image){
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>orderRegister/delete.do?id="+Id;
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>orderRegister/deleteAll.do',
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//修改
		function edit(Id){
			addOrEdit('<%=basePath%>orderRegister/goEdit.do?id='+Id,'编辑');
		}

...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="patient/patientList.do" method="post" name="Form" id="Form">
					...
					<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
						<thead>
							<tr>
								...
								<th class="center" >末次月经日期</th>
								<th class="center" >登记日期</th>
								<th class="center" >当日累计病人人数</th>
								<th class="center" >当日累计病人平均年龄</th>
						...
						<c:choose>
							<c:when test="${not empty patientList}">
								<c:if test="${QX.cha == 1 }">
								<c:forEach items="${patientList}" var="var" varStatus="vs">
									<tr>
										<td class='center' style="width: 30px;">
											<label><input type='checkbox' name='ids' class="ace" value="${var.id}" /><span class="lbl"></span></label>
										</td>
										<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td class="center">${var.patientName}</td>
										
										<td class="center">${var.hosNumber}</td>
										<c:if test="${var.patientSex == 1 }">
											<td class="center">男</td>
										</c:if>
										<c:if test="${var.patientSex == 2 }">
										...
										<td class="center">${var.patientCardNo}</td>
										<td class="center">${var.patientPhoneNumber}</td>
										<td class="center"><fmt:formatDate value="${var.patientBirth}" pattern="yyyy-MM-dd"/></td>
										<td class="center"><fmt:formatDate value="${var.lastYDate}" pattern="yyyy-MM-dd"/></td>
										<td class="center"><fmt:formatDate value="${var.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
										<td class="center">${var.patientCount}</td>
										<td class="center">${var.avgAge}</td>
	...
	<script type="text/javascript">
		...
		function del(Id,Image){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>patient/delete.do?id="+Id+"&Image="+Image;
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>patient/deleteAll.do',
						    	data: {DATA_IDS:str},
								dataType:'json',

...
//		this.addJob(JOBNAME, FHTIME, TABLENAME,TIMINGBACKUP_ID);				//添加任务
	...
	public Object changeStatus() throws Exception{
		...
		if(STATUS == 2){
			...
			String TABLENAME = pd.getString("TABLENAME");					//表名or整库(all)
			String TIMINGBACKUP_ID = pd.getString("TIMINGBACKUP_ID");		//任务数据库记录的ID
			this.addJob(JOBNAME, FHTIME, TABLENAME,TIMINGBACKUP_ID,jobType);		//添加任务
		}
	 ...
	 */
	public void addJob(String JOBNAME, String FHTIME, String TABLENAME, String TIMINGBACKUP_ID,String jobType){
		...
		if("2".equals(jobType)){
			...
			QuartzManager.addJob(JOBNAME,JOB_GROUP_NAME, JOBNAME, TRIGGER_GROUP_NAME, MessageQuartzJob.class, FHTIME ,parameter);
		} 
	}

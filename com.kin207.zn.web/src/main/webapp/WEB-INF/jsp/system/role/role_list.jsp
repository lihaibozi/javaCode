...
<body class="no-skin">
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-top:7px;">
								...
								<c:choose>
									<c:when test="${not empty roleList_z}">
										...
										<td style="width:30px;"><a onclick="roleButton('${var.ROLE_ID }','check_qx');" class="btn btn-warning btn-mini" title="分配审核权限"><i class="ace-icon fa fa-wrench bigger-110 icon-only"></i></a></td>
										</c:if>
	...
	<script type="text/javascript">
		...
		function roleButton(ROLE_ID,msg){
			...
			}else if(msg == 'check_qx'){
				Title = "授权审核权限(此角色下打勾的菜单拥有审核权限)";
			}
			 var diag = new top.Dialog();
			 diag.Drag = true;
			 diag.Title = Title;

...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="hospital/hospital.do" method="post" name="Form" id="Form">
						...
						<tr>
							<td>
								...
								<span class="input-icon">
									<input autocomplete="off" class="nav-search-input"  id="nav-search-input" type="text" name="keyword"  value="${pd.keyword}" placeholder="输入医院名称或首拼" />
								</span>
								</div>
							</td>
							<c:if test="${QX.cha == 1 }">
					...
					<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
						<thead>
							<tr>
								...
								<th class="center" >医院代码</th>
								<th class="center" >医院名称</th>
								<th class="center" >医院地址</th>
								<th class="center" >医院级别</th>
								<th class="center" >医院首拼</th>
								<th class="center" >医院联系人</th>
								<th class="center" >医院联系人电话</th>
						...
						<c:choose>
							<c:when test="${not empty hospitalList}">
								<c:if test="${QX.cha == 1 }">
								<c:forEach items="${hospitalList}" var="var" varStatus="vs">
									<tr>
										...
										<td class="center">${var.firstSpell}</td>
										<td class="center">${var.contactPerson}</td>
										<td class="center">${var.contactPhone}</td>
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			 addOrEdit('<%=basePath%>hospital/goAdd.do','新增');
		}
		...
		function del(Id,Image){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>hospital/delete.do?id="+Id+"&Image="+Image;
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>hospital/deleteAll.do',
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//修改
		function edit(Id){
			addOrEdit('<%=basePath%>hospital/goEdit.do?id='+Id,'编辑');
		}

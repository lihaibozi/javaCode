...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="shop/shopList.do" method="post" name="Form" id="Form">
						...
						<tr>
							<td>
								...
								<span class="input-icon">
									<input autocomplete="off" class="nav-search-input"  id="nav-search-input" type="text" name="keyword"  value="${pd.keyword}" placeholder="这里输入编码，名称" />
								</span>
								</div>
							</td>
							<c:if test="${QX.cha == 1 }">
					...
					<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
						<thead>
							<tr>
								...
								<th class="center" >店面图片</th>
								<th class="center" >联系人</th>
								<th class="center" >联系电话</th>
								<th class="center" >地址</th>
								<th class="center" >详细地址地址</th>
	...
	<script type="text/javascript">
		...
		function del(Id){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>shop/delete.do?id="+Id;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>shop/goEdit.do?id='+id;
			 diag.Width = 600;
			 diag.Height = 480;
			 diag.CancelEvent = function(){ //关闭事件
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>shop/deleteAll.do',
						    	data: {DATA_IDS:str},
								dataType:'json',
								cache: false,

...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="worktype/worktype.do" method="post" name="Form" id="Form">
					...
					<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
						...
						<c:choose>
							<c:when test="${not empty worktypeList}">
								<c:if test="${QX.cha == 1 }">
								<c:forEach items="${worktypeList}" var="var" varStatus="vs">
									<tr>
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			 addOrEdit('<%=basePath%>worktype/goAdd.do','新增');
		}
		...
		function del(Id,Image){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>worktype/delete.do?id="+Id+"&Image="+Image;
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>worktype/deleteAll.do',
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//修改
		function edit(Id){
			addOrEdit('<%=basePath%>worktype/goEdit.do?id='+Id,'编辑');
		}

...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="hosdep/hosdep.do" method="post" name="Form" id="Form">
						...
						<tr>
							<td>
								...
								<span class="input-icon">
									<input autocomplete="off" class="nav-search-input"  id="nav-search-input" type="text" name="keyword"  value="${pd.keyword}" placeholder="这里输入科室或医院名称" />
								</span>
								</div>
							</td>
							<c:if test="${QX.cha == 1 }">
					...
					<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
						...
						<!-- 开始循环 -->	
						<c:choose>
							<c:when test="${not empty hosdepList}">
								<c:if test="${QX.cha == 1 }">
								<c:forEach items="${hosdepList}" var="var" varStatus="vs">
									<tr>
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			 addOrEdit('<%=basePath%>hosdep/goAdd.do','新增');
		}
		...
		function del(Id,Image){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>hosdep/delete.do?id="+Id+"&Image="+Image;
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>hosdep/deleteAll.do',
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//修改
		function edit(Id){
			addOrEdit('<%=basePath%>hosdep/goEdit.do?id='+Id,'编辑');
		}

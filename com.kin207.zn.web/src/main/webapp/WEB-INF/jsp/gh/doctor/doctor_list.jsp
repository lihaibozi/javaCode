...
<body class="no-skin">
						...
						<tr>
							<td>
								...
								<span class="input-icon">
									<input autocomplete="off" class="nav-search-input"  id="nav-search-input" type="text" name="keyword"  value="${pd.keyword}" placeholder="这里输入医生姓名" />
								</span>
								</div>
							</td>
							<c:if test="${QX.cha == 1 }">
							...
							<c:when test="${not empty doctorList}">
								...
								<c:forEach items="${doctorList}" var="var" varStatus="vs">
									<tr>
										...
										<td class="center">${var.workId}</td>
										<td class="center">${var.cardNo}</td>
										<td class="center">${var.cellhone}</td>
										<td class="center">${var.proTitle}</td>
										<td class="center">${var.duty}</td>
	...
	<script type="text/javascript">
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>doctor/deleteAll.do',
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		function del(Id,Image){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>doctor/delete.do?id="+Id+"&Image="+Image;
		...
		//新增
		function add(){
			 addOrEdit('<%=basePath%>doctor/goAdd.do','新增');
		}
		
		//修改
		function edit(Id){
			addOrEdit('<%=basePath%>doctor/goEdit.do?id='+Id,'编辑');
		}
